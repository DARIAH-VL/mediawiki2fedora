...
my $importer = Catmandu->importer($mediawiki_importer);

{
    my $g = $importer->generate();
    my $short_n = $ns_map->{$g};
    if( $short_n ) {
        ...
        if( %extra ){
            ...
            $importer = Catmandu->importer($mediawiki_importer,args => { %{ $importer->args }, %extra  } );
